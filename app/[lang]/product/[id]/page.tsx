    ...
    <div className="bg-white dark:bg-neutral-900 min-h-screen">
      <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6 md:py-8">
        ...
        <nav className="mb-6 text-sm text-neutral-600 dark:text-neutral-400">
          <Link href={`/${locale}`} className="hover:text-purple-600 dark:hover:text-purple-400">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="flex flex-col">
            {/* Название и рейтинг */}
            <div className="mb-4">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-neutral-900 dark:text-neutral-100 mb-3">
                {product.name}
              </h1>
            ...
            <div className="mb-6">
              <div className="flex items-baseline gap-3 mb-2">
                <span className="text-3xl sm:text-4xl font-bold text-neutral-900 dark:text-neutral-100">
                  {product.price} {dictionary.currency}
                </span>
                {product.discount && (
                  <span className="text-lg sm:text-xl text-red-600 dark:text-red-400 font-semibold">
                    -{product.discount}%
                  </span>
                )}
              </div>
            ...
            <div className="mb-6 space-y-3">
              {product.model && (
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-neutral-600 dark:text-neutral-400 min-w-[100px]">
                    {dictionary.product?.model || "Модель"}:
                  </span>
                  <span className="text-sm text-neutral-900 dark:text-neutral-100">{product.model}</span>
                </div>
              )}
              {product.memory && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-neutral-900 dark:text-neutral-100">{product.memory}</span>
                </div>
              )}
              {product.color && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-neutral-900 dark:text-neutral-100">{product.color}</span>
                </div>
              )}
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-neutral-600 dark:text-neutral-400 min-w-[100px]">
